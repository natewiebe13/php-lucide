...
class IconTest extends TestCase
{
    ...
    private string $iconContent = '<polyline points="0 12 24 12" />';
    ...
    {
        $iconString = \sprintf(
            '<svg class="lucide lucide-%s" aria-hidden="true">%s</svg>',
            $this->icon->getName(),
            $this->iconContent
        );
    ...
    public function testAttributeFiltering(): void
    {
        ...
        $iconString = \sprintf(
            '<svg class="lucide lucide-%s" aria-hidden="true" width="24">%s</svg>',
            $this->icon->getName(),
            $this->iconContent
        );
    ...
    public function testAttributeEscaping(): void
    {
        ...
        $iconString = \sprintf(
            '<svg class="lucide lucide-%s" aria-hidden="true" data-title="Quote &quot; should be escaped">%s</svg>',
            $this->icon->getName(),
            $this->iconContent
        );
    ...
    {
        $iconTitleId = 'lucide-icon-title';

        $this->icon->setAltText('Example alt text');

        $iconString = \sprintf(
            <<<SVG
                <svg aria-labelledby="%s" class="lucide lucide-%s" role="img">
                  <title id="%s">%s</title>
                  %s
                </svg>
            SVG,
